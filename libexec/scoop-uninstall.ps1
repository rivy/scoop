...
if ($null -ne $args) { $args | foreach-object {
    ...
    if ($null -ne $versions) { $versions | foreach-object {
        ...
        # try { remove-item -r "\\?\$(resolve-path -literalpath $dir)" -ea stop -force ; info "'$dir' was removed" } ## only for $PSVersionTable.PSVersion -gt [System.Version]::Parse("3.0")
        try { remove-item -r -literalpath "$(resolve-path -literalpath $dir)" -ea stop -force ; info "'$dir' was removed" }
